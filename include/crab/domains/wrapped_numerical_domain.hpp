...
namespace crab {
    namespace domains {
        ...
        wrapped_numerical_domain<Domain1, Domain2> > {
        public:

            typedef abstract_domain<typename Domain1::number_t, typename Domain1::varname_t,
            wrapped_numerical_domain<Domain1, Domain2> > abstract_domain_t;
        ...
        public:
            ...
            typedef wrapped_numerical_domain<Domain1, Domain2> wrapped_numerical_domain_t;
...
#define signed_int
            ...
            wrapped_numerical_domain(domain_product2_t product) : _product(product) {
            }
            ...
            interval_t convert_to_valid_modulo_range(bool is_signed){
            //also need to know the modulo
            ...
            void reduce(bool is_signed) {
                ...
                auto interval_domain = inv2(); //i2 is interval_domain_t
                vector <std::pair<variable_t, interval_t>> intervals = interval_domain.to_interval_set();
                ...
                //                    return;
                //                }
                //                CRAB_WARN(" FIRST after reducing from the second", first);
            ...
            std::vector<interval_t> from_wrapped_to_intervals(variable_t v, wrapped_interval_t i, bool is_signed) {
                //TODO: improve the code considering if i is top
            ...
            void wrap_cond_exprs(Domain1& first, Domain2& second, linear_constraint_system_t csts, bool is_signed) {
                ...
                if (is_variable_lhs) {
                    cond_wrap_var_based_on_wrapped_interval(*(lhs_branch_cond.get_variable()), first, second, new_lcs, is_signed);
                } else {
                    cond_wrap_expr_based_on_wrapped_interval(new_lcs, first, second, is_signed);
                }
            }
             ...
             */
            void cond_wrap_var_based_on_wrapped_interval(variable_t v, Domain1& first, Domain2& second, linear_constraint_t csts, bool is_signed) {
                if (var_overflow(v, first, is_signed)) {
                    CRAB_WARN(v, " overflew: cond_wrap_var_based_on_wrapped_interval");
             ...
             */
            std::vector<Domain2> cond_wrap_exprs_var_based_on_wrapped_interval(variable_t v, Domain1& first, Domain2& second, bool is_signed) {
                std::vector<Domain2> dom2_elem;
                if (var_overflow(v, first, is_signed)) {
                    CRAB_WARN(v, " overflew: cond_wrap_exprs_var_based_on_wrapped_interval");
                    //call wrapping since it overflows
                    dom2_elem = get_disj_domains_based_on_wrapped_interval(v, first, second, is_signed);
                } else {
            ...
            std::vector<Domain2> wrap_exprs_single_var(variable_t v, Domain1& first, vector<Domain2>& second_list, bool is_signed) {
                ...
                for (Domain2 second : second_list) {
                    std::vector<Domain2> temp = cond_wrap_exprs_var_based_on_wrapped_interval(v, first, second, is_signed);
            ...
            //TODO: optimize this, don't consider vars which are already in the cond.

            void forget_overflow_vars(Domain1& first, Domain2& second, bool is_signed) {
                //get all variables from second
                if (second.is_bottom() || first.is_bottom()) return;
                linear_constraint_system_t lcs = second.to_linear_constraint_system();
                variable_set_t vars = lcs.variables();
                for (auto v : vars) {
                    //if var overflow do forget the vars.  var_overflow(v, first, is_signed);
                    if (first.get_wrapped_interval().is_top()) { //currently it is top, we need to forget it
                        std::vector<variable_t> artifical_vars;
                        artifical_vars.push_back(v);
            ...
            template <typename T>
            std::vector<variable_t> get_vars(T dom) {
                cout << "orig vars ========================= " << endl;
                std::vector<variable_t> var_vect;
                linear_constraint_system_t lcs = dom.to_linear_constraint_system();
                variable_set_t vars = lcs.variables();
                for (auto v : vars) {
                    var_vect.push_back(v);
                    CRAB_WARN(" ========== VAR ", v);
                }
                return var_vect;
            }
            ...
            void cond_wrap_expr_based_on_wrapped_interval(linear_constraint_t branch_cond, Domain1& first, Domain2& second, bool is_signed) {
                ...
                if (expr_overflow(lhs, first, is_signed)) {
                    ...
                    second = wrap_var_in_all_domains<Domain2>(var_new, first, res_vars_wrap, new_cond, is_signed);
                    std::vector<variable_t> artifical_vars;
                    artifical_vars.push_back(var_new);
                    CRAB_WARN("new var / SECOND before: cond_wrap_expr_based_on_wrapped_interval  ", var_new, " / ", second);
                    //TODO: check why projection was not working
                    //crab::domains::domain_traits<Domain2>::project(second, orig_vars.begin(), orig_vars.end());
                    crab::domains::domain_traits<Domain2>::forget(second, artifical_vars.begin(), artifical_vars.end());
                    //since in our case, the first domain is the wrapped-interval, forget will be same as project
                    crab::domains::domain_traits<Domain1>::forget(first,
                            artifical_vars.begin(), artifical_vars.end());
                    CRAB_WARN("SECOND: cond_wrap_expr_based_on_wrapped_interval ", second);
                } else {
            ...
            template <typename T>
            T wrap_var_in_all_domains(variable_t v, Domain1 first, vector<T> domains_2, linear_constraint_t new_cond, bool is_signed) {
                T res = T::bottom();
                for (auto dom2 : domains_2) {
                    wrap_a_var_based_on_wrapped_interval(v, first, dom2, new_cond, is_signed);
                    res |= dom2;
                }
                return res;
            }
            ...
            void wrap_a_var_based_on_wrapped_interval(variable_t var, Domain1& first, Domain2& second, linear_constraint_t csts, bool is_signed) {
                second = project_single_var_numerical_domain(var, second);
                wrapped_interval_t i_var = first.get_wrapped_interval(var);
            ...
            std::vector<Domain2> get_disj_domains_based_on_wrapped_interval(variable_t var, Domain1& first, Domain2& second, bool is_signed) {
                std::vector<Domain2> dom2_elem;
                second = project_single_var_numerical_domain(var, second);
                wrapped_interval_t i_var = first.get_wrapped_interval(var);
                Domain2 second_cp = second; //copy of the second domain
                if (!i_var.is_top()) {
                    ...
                    for (auto lcs : lcss) {
                        Domain2 temp = second_cp;
                        temp += lcs;
                        dom2_elem.push_back(temp);
                    }
                } else {
                    //if it is not top, we can do [0, pos max] join [-neg min, neg max]

                    vector<linear_constraint_system_t> bounds = get_var_bounds_hemispherewise(var, is_signed);
                    Domain2 second_cp_1 = second_cp;

                    second_cp += bounds.front();
                    second_cp_1 += bounds.back();
                    dom2_elem.push_back(second_cp);
                    dom2_elem.push_back(second_cp_1);
                }
                return dom2_elem;
            }
            ...
            void wrap_single_var_SK(variable_t var, Domain2& second, linear_constraint_system_t csts, long threshold = 16) {
                // FIXME: this is assuming number_t=z_number
                ...
                CRAB_WARN("interval ", var_interval);
                ...
                linear_constraint_system_t vars_bounds = get_var_bounds(var);
                ...
                if (!var_interval.lb().is_finite() || !var_interval.ub().is_finite() || (upper_quad_index - lower_quad_index) > threshold) {
                    ...
                    project_single_var_numerical_domain(var, second);
            ...
            Domain2 & project_single_var_numerical_domain(variable_t var, Domain2 & second) {
            ...
            Domain2& update_var_in_domain(Domain2& numerical_domain, variable_t var, int quotient, number_t modulo) {
                ...
                CRAB_WARN("about to rename ", frm_vars, " to ", to_vars);
                numerical_domain.rename(frm_vars, to_vars);
                CRAB_WARN("after renaming");
                ...
                return project_single_var_numerical_domain(var_new, numerical_domain);
            }
        ...
        public:
            ...
            void operator+=(linear_constraint_system_t csts) {
                bool is_signed = signed_world();
                //CRAB_WARN("FIRST/SECOND/COND ", this->_product.first(), " / ", this->_product.second(), " / ", csts);
                wrap_cond_exprs(this->_product.first(), this->_product.second(), csts, is_signed); //makes second domain sound wrt modular arithmetic, so the following operation is sound
                //forget_overflow_vars(this->_product.first(), this->_product.second(), is_signed);
        ...
        template<typename Domain1, typename Domain2>
        class domain_traits <wrapped_numerical_domain<Domain1, Domain2> > {
        public:
